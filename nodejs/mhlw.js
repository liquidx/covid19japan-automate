...
const getSummaryTableFromReport = async (fetch, reportUrl, credentials) => {
  return fetch(reportUrl)
    ...
    .then(async (html) => {
      ...
      const image = ImageDataURI.decode(tableImage.attr("src"));
      const response = await textDetectionForImage(
        credentials,
        image.dataBase64
      );
      const detectionResult = response.responses[0];

      return { date, image, detectionResult };
    });
};
